...
class SupplierNameDTO : public oatpp::DTO
{
	...
	DTO_FIELD_INFO(supplierId) {
		info->description = ZH_WORDS_GETTER("supplier-manage.query-supplier-name-list.field.supplier-id");
	}
	// 供应商名称
	DTO_FIELD(String, supplierName);
	DTO_FIELD_INFO(supplierName) {
		info->description = ZH_WORDS_GETTER("supplier-manage.query-supplier-name-list.field.supplier-name");
	}
};
